...
// Rate limiting
app.use("/api/auth/login", rateLimit({ windowMs:15*60*1000, max:20, message:{error:"Demasiados intentos."} }));
app.use("/api", rateLimit({ windowMs:60*1000, max:300, message:{error:"Demasiadas solicitudes"} }));
...
app.use((err, req, res, _next) => { console.error(err); res.status(500).json({ error:"Error interno" }); });
